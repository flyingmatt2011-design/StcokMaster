...
const fieldTitleMap: Record<string, string> = {
  ...
  LITELLM_ANALYSIS_TIMEOUT_SECONDS: '分析模型超时（秒）',
  LITELLM_FAST_MODEL_TIMEOUT_SECONDS: '快速模型超时（秒）',
  LITELLM_QUALITY_FALLBACK_TIMEOUT_SECONDS: '高质量备用超时（秒）',
  LITELLM_CIRCUIT_FAILURE_THRESHOLD: '模型熔断失败阈值',
  LITELLM_CIRCUIT_COOLDOWN_SECONDS: '模型熔断冷却（秒）',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  LITELLM_ANALYSIS_TIMEOUT_SECONDS: '普通分析模型单次最多等待 90 秒，超时后切换到下一备用模型。',
  LITELLM_FAST_MODEL_TIMEOUT_SECONDS: '名称含 flash 的快速模型单次最多等待 60 秒。',
  LITELLM_QUALITY_FALLBACK_TIMEOUT_SECONDS: 'Claude/Sonnet 等高质量最终备用单次最多等待 120 秒。',
  LITELLM_CIRCUIT_FAILURE_THRESHOLD: '同一模型连续失败达到阈值后暂时跳过，避免拖慢整批任务。',
  LITELLM_CIRCUIT_COOLDOWN_SECONDS: '熔断模型经过该冷却时间后允许再次探测。',
