...
- [修复] Kronos 在本地历史少于 120 根时于评分定稿后独立补取日线，避免功能已开启但报告只能显示数据不足；资金流实时接口超时后复用本交易日最近成功结果，减少相同盘后输入因瞬时缺失产生的建议抖动，评分与护栏规则不变，并将对应数据源兜底文件纳入上游同步强需求保护。
- [改进] 将 Kronos 后置预测接入登记为上游算法同步强需求，使用三方合并吸收兼容更新、冲突时整文件保留 StockMaster 版本，并在设置页显示保护文件数量。
- [新功能] A 股个股最终报告可选展示 Kronos 实验性日线预测；预测在现有评分、护栏和操作建议定稿后独立运行，缺依赖或推理失败时按 fail-open 降级，不进入 LLM 输入且不参与评分。
- [改进] 多模型分析增加按模型独立超时、连续失败熔断和自适应并发恢复，移除 Router 与外层 fallback 的重复重试；保持提示词、分析策略和评分规则不变。
- [改进] 桌面端在进入 LLM 前持久化已准备输入，后台重启后未完成任务可从模型与报告整理阶段继续，并在运行流记录分模型尝试、结果校验耗时和完整评分轨迹。
- [改进] A 股免费日线连接异常改为 15 秒快速失败和 120 秒短期熔断；首页大盘快照增加缓存、并发合并及批量分析期暂停刷新，减少免费数据源争抢。
- [修复] 高频腾讯批量行情成功日志降为调试级，桌面后端禁用第三方进度条输出，保留失败告警并降低日志噪声。
- [修复] 后端算法更新同步范围补齐 Jinja 报告模板，并在候选切换前编译校验变更模板及其加载路径，避免指数分析已更新但聚合报告缺少数据源展示。
- [修复] 桌面端启动时的大盘上下文预热改为仅复用缓存或历史记录，缺少可复用记录时不再自动调用 LLM，手动大盘复盘与个股分析中的正常生成能力保持不变。
