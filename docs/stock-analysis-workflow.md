...
flowchart TD
    ...
    U --> U1{Kronos 预测已启用?}
    U1 -->|是| U2[独立预测未来日线<br/>不进入评分和操作建议]
    U1 -->|否| V[保存分析历史、资讯证据<br/>上下文快照和运行诊断]
    U2 -->|成功或降级| V
    V --> W[生成结构化报告]
    W --> W1[评分 / 建议 / 趋势]
    W --> W2[核心结论 / 风险 / 催化]
    ...
    class U1,U2,V,W,W1,W2,W3,W4,W5,Y done;
    class X,Z fail;
```
...
- Kronos 在评分、护栏和最终动作全部定稿后才运行；预测只保存和展示，不进入 LLM 输入、不参与评分或建议，失败时主分析照常完成。
...
| 96% | 可选预测 | 已启用时生成 Kronos 实验性日线预测，不计入评分 |
