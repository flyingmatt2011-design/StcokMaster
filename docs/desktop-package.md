...
- 后端启动时只从缓存或历史记录预热最近交易日的大盘上下文；如果没有可复用记录则跳过，不会因打开桌面应用自动调用 LLM。手动“大盘复盘”和个股分析仍可按原流程生成所需上下文
...
通过 `StockMaster-Start.bat` 启动时，设置页可以检测原项目的后端分析算法更新，并在发现候选版本后显示“同步更新”按钮。同步只下载允许更新的后端文件，不同步 `apps/` 等 UI 目录。对于源仓库与 StockMaster 同时修改的文件，程序使用固定上游基线执行三方合并：无冲突改动自动兼容；只要文件内出现真实冲突，就保留完整的 StockMaster 本地文件，避免跨代码块混合形成语义残缺，并在设置页记录合并数量与冲突数量。切换前会校验所有受影响 Python 文件、导入 FastAPI 应用、使用内存数据库初始化 `DatabaseManager`，并在重启后检查 `/api/health`；校验或健康检查失败时会自动恢复上一个可运行版本。
...
- 应用启动时不会主动检查 GitHub Releases，也不会因为上游桌面端版本变化弹出全栈升级提醒
- 用户在 `系统设置 -> 版本信息 -> 桌面端更新` 中手动检查后，Windows NSIS 安装版才会通过内置 GitHub 更新源下载新版本；下载完成后弹出一次性提醒，用户确认后静默重启并安装
- 自动更新静默安装会复用当前安装目录；如果用户安装时选择了非默认目录或带空格目录，后续自动更新仍会覆盖同一目录
- `系统设置 -> 版本信息` 中的“桌面端更新”区域可手动检查更新；若更新已下载，会展示“重启安装”操作
- Windows 免安装包、开发态和 macOS DMG 的手动检查仍保持“提醒 + 跳转下载页”的兼容路径，不会因为网络失败而阻断桌面端启动
...
- 桌面端会把尚未完成的标准个股分析任务写入用户数据目录下的 `unfinished-analysis-tasks.json`；应用异常退出后再次启动，后端会恢复当前任务，首页会接着处理剩余自选股队列。正常完成、失败或用户选择“分析完当前股票后停止”后会清理对应记录。
- 恢复记录只保存股票代码、任务参数和队列进度，不保存 API Key、模型凭据或完整分析报告。
- 桌面端连续分析多只股票时默认复用同一个线程安全数据抓取器的短期缓存，减少重复行情、K 线、名称和基本面请求。该优化不改变数据源顺序、fallback、技术指标、LLM 提示词、评分规则或建议判断；LLM 调用本身仍按原流程执行。
- 若需临时回退共享缓存，可在启动环境中设置 `STOCKMASTER_SHARED_FETCHER_CACHE=false`；若需清除异常退出恢复队列，请先退出 StockMaster，再删除用户数据目录中的 `unfinished-analysis-tasks.json`。
...
如果通过 `StockMaster-Start.bat` 启动开发版，脚本会先自动补齐 Python 依赖。若手动执行 Electron 或 `npm run dev`，请在仓库根目录执行 `python -m pip install -r requirements.txt` 后再启动。
